...
def main():
    ...
    T=300
    back_dielect=1
    qpoint=[[qx,qy,0],[0.5*qx,0.5*qy,0]]
    #########calulating with lindhard function
    plascal = tb.Lindhard(cell,enerange,step,mesh,mu,T,back_dielect)
    omegas,dyn = plascal.DP_qpoints(qpoint)
    for i in range(len(qpoint)):
        plt.plot(np.array(omegas)/t, -dyn.imag[i,:],label="qa=%s" % (1*0.15))
    plt.xlabel(r'$\hbar * \omega$/t0')
    plt.ylabel(r'-Im($\Pi) (eV^{-1}nm^{-2})$')
    plt.xlim((0,5))
    plt.legend()
    plt.savefig("lindhard_im_dyn_pol.eps",dpi=300)
    plt.close()
    np.savetxt("dyn.dat",-dyn.imag)
